...
export const createShuttleStore: StateCreator<ShuttleStore> = (set, get) => ({
  recentWallet: null,
  wallets: [],
  ...
  getWallets: (filters?: { providerId?: string; chainId?: string }) => {
    ...
    if (filters) {
      if (filters.providerId) {
        wallets = wallets.filter((wallet: WalletConnection) => wallet.providerId === filters.providerId);
      }
      if (filters.chainId) {
        wallets = wallets.filter((wallet: WalletConnection) => wallet.network.chainId === filters.chainId);
      }
    }
    return wallets;
  },
